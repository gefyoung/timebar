...
const triggerStaticRegeneration = async (options) => {
    ...
    const { SQSClient, SendMessageCommand } = await Promise.resolve().then(function () { return require('./index-a033b08c.js'); });
...
const s3StorePage = async (options) => {
    const { S3Client } = await Promise.resolve().then(function () { return require('./S3Client-71a1a276.js'); });
    ...
    const { PutObjectCommand } = await Promise.resolve().then(function () { return require('./PutObjectCommand-17deb54e.js'); });
    await Promise.all([
...
const handleOriginRequest = async ({ event, manifest, prerenderManifest, routesManifest }) => {
    ...
    if (route.isPublicFile) {
        ...
        return staticRequest(request, file, `${routesManifest.basePath}/deploy-${manifest.buildId}/public`);
    }
    if (route.isStatic) {
...
const handleOriginResponse = async ({ event, manifest, prerenderManifest, routesManifest }) => {
    ...
    if (fallbackRoute.isStatic) {
        ...
        const { GetObjectCommand } = await Promise.resolve().then(function () { return require('./GetObjectCommand-1e69dc8c.js'); });
